        ...
        if (isArray(options_1) && options_1.length) {
            options_1.forEach(function (_a, index) {
                ...
                if ((ref && isDetached(ref)) || forceDelete) {
                    var mutationWatcher_1 = ref.mutationWatcher;
...
function useForm(_a) {
    ...
    var handleSubmit = useCallback(function (callback) { return function (e) { return __awaiter(_this, void 0, void 0, function () {
        var fieldErrors, fieldValues, fields, _a, errors, values, _b, errors, values, key, field;
        var _this = this;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
    ...
    var reset = function (values) {
        ...
        Object.values(resetFieldArrayFunctionRef.current).forEach(function (resetFieldArray) { return isFunction(resetFieldArray) && resetFieldArray(values); });
        resetRefs();
        if (values) {
...
var appendId = function (value) { return (__assign(__assign({}, value), (value.id ? {} : { id: generateId() }))); };
...
function getIsFieldsDifferent(referenceArray, differenceArray) {
    ...
    for (var i = 0; i < referenceArray.length; i++) {
        ...
        var _a = referenceArray[i], id = _a.id, data = __rest(_a, ["id"]);
...
function useFieldArray(_a) {
    ...
    var _b = control || methods.control, resetFieldArrayFunctionRef = _b.resetFieldArrayFunctionRef, fieldArrayNamesRef = _b.fieldArrayNamesRef, fieldsRef = _b.fieldsRef, defaultValuesRef = _b.defaultValuesRef, unregister = _b.unregister, isDirtyRef = _b.isDirtyRef;
    var memoizedDefaultValues = React.useMemo(function () { return get(defaultValuesRef.current, name, []); }, 
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [name]);
    var _c = __read(React.useState(mapIds(memoizedDefaultValues)), 2), fields = _c[0], setField = _c[1];
    ...
    var prepend = function (value) {
        resetFields();
        setField(__spread([appendId(value)], fields));
    };
    var append = function (value) {
        isDirtyRef.current = true;
        setField(__spread(fields, [appendId(value)]));
    };
    var remove = function (index) {
        var data = isUndefined(index)
            ? []
            : __spread(fields.slice(0, index), fields.slice(index + 1));
        resetFields(data);
        setField(data);
    };
    var insert = function (index, value) {
        resetFields();
        setField(__spread(fields.slice(0, index), [
            appendId(value)
        ], fields.slice(index)));
    };
    var swap = function (indexA, indexB) {
        var _a;
        _a = __read([fields[indexB], fields[indexA]], 2), fields[indexA] = _a[0], fields[indexB] = _a[1];
        resetFields(fields);
...
var Controller = function (_a) {
    ...
    var methods = useFormContext();
    var _j = control || methods.control, defaultValuesRef = _j.defaultValuesRef, setValue = _j.setValue, register = _j.register, unregister = _j.unregister, errors = _j.errors, _k = _j.mode, isOnSubmit = _k.isOnSubmit, isOnBlur = _k.isOnBlur, _l = _j.reValidateMode, isReValidateOnBlur = _l.isReValidateOnBlur, isReValidateOnSubmit = _l.isReValidateOnSubmit, isSubmitted = _j.formState.isSubmitted, fieldsRef = _j.fieldsRef, fieldArrayNamesRef = _j.fieldArrayNamesRef;
...
var ErrorMessage = function (_a) {
    var InnerComponent = _a.as, errors = _a.errors, name = _a.name, children = _a.children;
    var methods = useFormContext();
    var _b = get(errors || methods.errors, name, {}), message = _b.message, types = _b.types;
    if (!message) {
        return null;
    }
    var props = {
        children: children ? children({ message: message, messages: types }) : message,
    };
    return InnerComponent ? (React.isValidElement(InnerComponent) ? (React.cloneElement(InnerComponent, props)) : (React.createElement(InnerComponent, __assign({}, props)))) : (React.createElement(React.Fragment, __assign({}, props)));
};
